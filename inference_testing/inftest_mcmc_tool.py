...
def inference_test_plot(alpha_vec, prob, prob_lo, prob_hi, title=None,savename='convergence_test.pdf'):
    ...
    plt.xlim((-0.01, 1.01))
    plt.ylim((-0.01, 1.01))
    plt.xlabel(r'$P_{{\rm inf}}$', fontsize=16)
    plt.ylabel(r'$P_{{\rm cov}}$', fontsize=16)
    plt.title(title)
...
def run_inference_test_GMM(samples, truths, ngauss, alpha_vec, nGM_samp=10000, seed_or_rng=None, C_ge_lnP_true=None,
    ...
    for imock in range(nmock):
        ...
        if imock == 0:
            inf_test_marg = np.zeros((nmock,) + inf_test_marg0.shape, dtype=bool) #bug , would the 2nd term on right be inf_test_marg0? #
